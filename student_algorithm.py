...
class TradingBot:
    ...
    def _on_market_data(self, ws, message: str):
        ...
        try:
            ...
            # Log progress every 100 steps with latency stats
            if self.current_step % 100 == 0 and self.step_latencies:
        ...
        try:
            ...
            if msg_type == "AUTHENTICATED":
                ...
                if order_id in self.order_send_times:
                    fill_latency = (recv_time - self.order_send_times[order_id]) * 100  # ms
...
Examples:
  ...
  Deployed server (HTTPS):
    ...
    parser.add_argument("--name", required=True, help="Quackonomics")
